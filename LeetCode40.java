...
public class LeetCode40 {
     ...
     * }
     * 
     * private void helper(int index, int[] nums, int tempSum, int target,
     ...
     * }
     * 
     * for (int i = index; i < nums.length; i++) { // 怎么理解？
     * 第一层从index个元素开始往后找，第二层从第二个元素开始找
     * if (visited[i])
     * continue;
     * if (i > 0 && nums[i] == nums[i - 1] && !visited[i - 1]) continue;
     * // why do we need this line in this question? this line skip the duplicate
     * elements
     * tempSum += nums[i];
     * tempResult.add(nums[i]);
     * visited[i] = true;
